...
from transformers.modeling_outputs import BaseModelOutputWithPast, MoeCausalLMOutputWithPast,QuestionAnsweringModelOutput, SequenceClassifierOutputWithPast, TokenClassifierOutput,MoeModelOutputWithPast
from transformers.cache_utils import Cache, DynamicCache, SlidingWindowCache, StaticCache
logger = logging.get_logger(__name__)
class DollyMoERMSNorm(nn.Module):
    '''
...
class DollyMoEMLP(nn.Module):
    '''
    DollyMoEMLP层的实现, code from Qwen3
...
class DollyMoERotaryEmbedding(nn.Module):
    def __init__(self, config: DollyMoEConfig, device=None):
        super().__init__()
...
class DollyMoEAttention(nn.Module):
    """
    Attention in transformer
    """
    def __init__(self, config: DollyMoEConfig, layer_idx: int):
        super().__init__()
        self.config = config
        ...
        self.q_norm = DollyMoERMSNorm(self.head_dim, eps=config.rms_norm_eps)
        self.k_norm = DollyMoERMSNorm(self.head_dim, eps=config.rms_norm_eps) 
        #支持滑动窗口
...
class DollyMoESparseMoeBlock(nn.Module):
    def __init__(self, config):
        ...
        self.experts = nn.ModuleList(
            [DollyMoEMLP(config.hidden_size, config.intermediate_size, config.hidden_act) for _ in range(self.num_experts)]
        )
...
class DollyMoEDecoderLayer(nn.Module):
    def __init__(self, config: DollyMoEConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = DollyMoEAttention(config=config, layer_idx=layer_idx)
        if (layer_idx not in config.mlp_only_layers) and (
            config.num_experts > 0 and (layer_idx + 1) % config.decoder_sparse_step == 0
        ):
            self.mlp = DollyMoESparseMoeBlock(config)
        else:
            self.mlp = DollyMoEMLP(config.hidden_size, config.intermediate_size, config.hidden_act)
        self.input_layernorm = DollyMoERMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = DollyMoERMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class DollyMoEPreTrainedModel(PreTrainedModel):
    config_class = DollyMoEConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["DollyMoEDecoderLayer"]
...
class DollyMoEModel(DollyMoEPreTrainedModel):
    def __init__(self, config: DollyMoEConfig):
        ...
        self.layers = nn.ModuleList(
            [DollyMoEDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        #RMSNorm层
        self.norm = DollyMoERMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        #RoPE层
        self.rotary_emb = DollyMoERotaryEmbedding(config=config)
...
class DollyMoEForCausalLM(DollyMoEPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = DollyMoEModel(config)
        self.vocab_size = config.vocab_size
...
class DollyMoEForSequenceClassification(DollyMoEPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class DollyMoEForTokenClassification(DollyMoEPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.model = DollyMoEModel(config)
...
class DollyMoEForQuestionAnswering(DollyMoEPreTrainedModel):
    base_model_prefix = "transformer"

    def __init__(self, config):
        super().__init__(config)
        self.transformer = DollyMoEModel(config)
...
__all__ = [
    "DollyMoEForCausalLM",
    "DollyMoEForQuestionAnswering",
    "DollyMoEModel",
    "DollyMoEPreTrainedModel",
    "DollyMoEForSequenceClassification",
    "DollyMoEForTokenClassification",
]